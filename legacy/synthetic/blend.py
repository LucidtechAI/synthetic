...
# The below poisson blending functions are courtesy of:
    ...
    for num_layer in range(img_target.shape[2]):
        ...
        for y in range(region_size[0]):
            for x in range(region_size[1]):
                ...
                if not img_mask[y, x]:
                    b[index] = t[index]
                # else:  # attempt at mixing gradients - probably must be done properly and is inappropriate here
                #     if np.abs(b[index]) < np.abs(bt[index]):
                #         b[index] = bt[index]
