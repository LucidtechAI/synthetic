...
def main():
    ...
    parser.add_argument('--font-size-range', type=str, default='20:40')
    ...
    with open(tmpl_path) as f:
        tmpl_dict = yaml.load(f)
        img_path = pathlib.Path(tmpl_dict['path']).relative_to(tmpl_path.parent)
        img = PIL.Image.open(str(tmpl_path.parent / img_path))

    box_to_text_dict = {}
    for label in tmpl_dict['labels']:
        for box in tmpl_dict['labels'][label]['boxes']:
            box = tuple([box[k] for k in ['xmin', 'ymin', 'xmax', 'ymax']])
            box_to_text_dict[box] = tmpl_dict['labels'][label]['text']

    ret_img = doodle_on(img, box_to_text_dict, fonts_by_size)
    ret_img.save(tmpl_dict['dest'])
